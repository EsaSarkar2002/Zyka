...
namespace Zyka.Controllers
{
    public class CustomerController : Controller
    {
...

        {
            ...
            var userName = _context.Users.Where(u => u.UserId == userId).Select(u => u.UserName).First();
...

        {
...

            {
                ...
                // No id provided - show generic confirmation page or redirect to reservation page
            ...
            var reservation = _context.Reservations
                ...
                .FirstOrDefault(r => r.ReservationId == reservationId.Value);
...

            {
                ...
                TimeSlotText = reservation.TimeSlot?.DisplayText ?? "N/A",

                Guests = reservation.NumberOfGuests,

                TableCategory = reservation.Table?.Category.ToString() ?? "N/A",

                TableNumber = reservation.Table?.TableNumber ?? "N/A",

                PaymentAmount = reservation.Payment?.Amount ?? 0m,

                PaymentMethod = reservation.Payment != null ? reservation.Payment.PaymentMethod.ToString() : "N/A",
        ...
        public IActionResult Support()
        {
            ...
            var userName = _context.Users.Where(u => u.UserId == userId).Select(u => u.UserName).FirstOrDefault();
        ...
        public IActionResult GetAvailableTimeSlots([FromBody] TimeSlotAvailabilityDto request)
        {
            ...
            {
                slotIds = _context.TimeSlots
                .Where(ts => ts.IsActive && ts.StartTime>currentTime)
                .Select(ts => ts.TimeSlotId)
                .ToList();
            }
